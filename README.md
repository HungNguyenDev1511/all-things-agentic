...
Idempotency — prevent duplicate writes after retry/resume.

Durable resumability — persist and resume interrupted workflows across process restarts.

REST API — expose migration jobs, approval, status, and artifacts.

Frontend — upload → progress → approval → final report.

Cloud Run deployment — provide visible Google Cloud deployment evidence.

Evals / regression tests — clean, ambiguous, invalid, malformed, and mixed migration scenarios.

Demo video (~4 minutes) — problem → live workflow → human approval → output/reconciliation → Google Cloud proof.
